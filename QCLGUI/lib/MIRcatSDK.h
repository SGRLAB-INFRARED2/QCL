...
/* Topic: Documentation and Code Conventions
	...
	- Every function the SDK starts with *MIRcatSDK_*. This prefix is omitted from the much of the documentation.
	- Since most functions return <Error Codes>, functions that require data output use out parameters. The user must
		pass a pointer with enough storage space to hold the data. Out parameters are noted in the documentation by
		*(out)*.
*/
#ifdef WIN32_EXPORT
...
*/
	MIRCAT_LIB uint32_t MIRcatSDK_GetAPIVersion(uint16_t * papiVersionMajor, uint16_t * papiVersionMinor, uint16_t * papiVersionPatch);
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If setting communication type if successful.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the controller is not yet initialized.
*		MIRcatSDK_RET_UNSUPPORTED_TRANSPORT - *[User Error]* If the user specified `commType` is invalid.
*
*	See Also:
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If setting the serial port is successful.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the controller is not yet initialized.
*
*	See Also:
...
*	Creates new MIRcat object.  If a previous call has been made to MIRcatSDK_DeInitialize(), this function should be called before trying to call other MIRcatSDK functions.
*
*	Returns:
...
*	Returns bool value indicating if MIRcat object has been created. The object is destroyed following a call to MIRcatSDK_DeInitialize(). If it is destroyed it must be created before any SDK function calls will be valid. A call to MIRcatSDK_Initialize() will also create the object if it has been destroyed.
*
*	Parameters:
*		pbMIRcatObjectCreated - (out) The current MIRcat controller object.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS*.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the MIRcat controller is successfully initialized.
*		MIRcatSDK_RET_INITIALIZATION_FAILURE - *[System Error]* If MIRcat controller Initialization failed.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_Initialize();

/* Function: DeInitialize
*	Disconnect and clean up ports/memory/threads associated with initializing the MIRcatSDK. A call to this function requires a call to MIRcatSDK_CreateMIRcatObject() or MIRcatSDK_Initialize() before any subsequent calls to any SDK functions.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If DeInitializing the MIRcat controller object was successful.
* 		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If user tries to DeInitialize MIRcat object before controller initialized.
*/
...
*
*	Parameters:
*		pszModelNumber - (out) Pointer to character array that will contain model number after calling the function.  This array should be at least 24 bytes.
*		bSize - Size of pszModelNumber in bytes.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If retrieving the model number is successful.
*		MIRcatSDK_RET_NOT_INITIALIZED - If the controller is not yet initialized.
*		MIRcatSDK_RET_BUFFER_TOO_SMALL - *[User Error]* If the user specified buffer is too small for the character array.
*/
...
*
*	Parameters:
*		pszSerialNumber - (out) Pointer to character array that will contain serial number after calling the function.  This array should be at least 24 bytes.
*		bSize - Size of pszSerialNumber in bytes.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If retrieving the model number is successful.
*		MIRcatSDK_RET_NOT_INITIALIZED - If the controller is not yet initialized.
...
*		pbUnits - (out) Units for the min/max wavelength of the MIRcat system.
*
*	See Also:
*		<Units>
*
...
*
*	Parameters:
*		pbConnected - (out) Bool value that indicates if the API is connected to the MIRcat system.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*
*	Parameters:
*		pbSet - (out) Bool value that indicates if the interlock circuit is closed.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*
*	Parameters:
*		pbSet - (out) Bool value that indicates if the key switch is in the ON position.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*
*	Parameters:
*		pbIsOn - (out) Bool value that indicates if the laser is currently emitting light.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system is successfully able to return a value.
*		MIRcatSDK_RET_NOT_INITIALIZED - If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_COMM_ERROR - If the MIRcat controller is unable to Read Info regarding the light from the system.
*/
...
*
*	Parameters:
*		pbIsArmed - (out) Bool value that indicates if the laser is armed
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*
*	Parameters:
*		pbIsError - (out) Bool value that indicates if there is a system error.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*	Attempt to clear system error.  If the error cannot be cleared it is likely a serious system error.
*
*	Parameters:
*		pbErrorCleared - (out) Bool value that indicates the error could be cleared.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*
*	Parameters:
*		pbIsAtSetTemperature - (out) Bool value that indicates if the TECs are at the set temperatures.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*
*	Parameters:
*		pwErrorWord - (out) 16-bit error code.  See user's manual for a list of error codes.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*
*	Parameters:
*		pbDisplayUnits - (out) Display units for wavelength in the laser settings.
*
*	See Also:
*		<Units>
*
...
*
*	Parameters:
*		pbIsScanInProgress - (out) Bool value that indicates if the scan is in progress.
*		pbIsScanActive - (out) Bool value that indicates if the scan is active.
*		pbIsScanPaused - (out) Bool value that indicates if the scan is paused.
*		pwCurScanNum - (out) Current scan number in repeated scan sequence.
...
*		pbIsTECInProgress - (out) Bool value that indicates if the laser is waiting for a TEC to get to the target temperature before firing.
*		pbIsMotionInProgress - (out) Bool value that indicates if a QCL is currently tuning.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_GetScanStatus( bool * pbIsScanInProgress, bool * pbIsScanActive, bool * pbIsScanPaused,
...
*
*	Parameters:
*		bWaitProcTrig - (out) Bool value that indicates whether the system is waiting for a user trigger.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*
*	Parameters:
*		pbActiveQcl - (out) The QCL that is active during this part of the scan/tune.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully arms the laser.
*		MIRcatSDK_RET_NOT_INITIALIZED - If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_LASER_ALREADY_ARMED - *[User Error]* If the user attempts to arm the laser when it has already been armed.
*		MIRcatSDK_RET_INTERLOCKS_KEYSWITCH_NOTSET - *[User Error]* If the interlock status is not set or the key switch is not set.
*		MIRcatSDK_RET_ARMDISARM_FAILURE - *[System Error]* If the system fails to arm the laser.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully disarms the laser.
*		MIRcatSDK_RET_NOT_INITIALIZED - If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_LASER_ALREADY_ARMED - *[User Error]* If the user attempts to disarm the laser when it has already been disarmed.
*		MIRcatSDK_RET_INTERLOCKS_KEYSWITCH_NOTSET - *[User Error]* If the interlock status is not set or the key switch is not set.
*		MIRcatSDK_RET_ARMDISARM_FAILURE - *[System Error]* If the system fails to disarm the laser.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_DisarmLaser();

/* Function: ArmDisarmLaser
*	Toggle the armed state of the laser based on the current state (i.e., if the laser is disarmed, this command will arm it and vice versa).
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully toggled laser arming.
*		MIRcatSDK_RET_NOT_INITIALIZED - If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_INTERLOCKS_KEYSWITCH_NOTSET - *[User Error]* If the interlock status is not set or the key switch is not set.
*		MIRcatSDK_RET_ARMDISARM_FAILURE - *[System Error]* If the system fails to toggle laser arming.
*/
...
*		MIRcatSDK_RET_NOT_INITIALIZED - If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_STOP_SCAN_FAILURE - *[System Error]* If the system fails to successfully stop the scan in progress.
*		MIRcatSDK_RET_NO_SCAN_INPROGRESS - *[User Error]* If the user attempts to stop a scan when there is no current scan in progress.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_StopScanInProgress();

/* Function: PauseScanInProgress
*	Pauses the current scan.  This function sends a pause command to the laser, but currently the laser does not support pausing of a scan.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system is successfully able to pause the scan in progress.
*		MIRcatSDK_RET_NOT_INITIALIZED - If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_PAUSE_SCAN_FAILURE - *[System Error]* If the system fails to pause the scan in progress.
*		MIRcatSDK_RET_NO_SCAN_INPROGRESS - *[User Error]* If the user attempts to pause a scan when there is no current scan in progress.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_PauseScanInProgress();

/* Function: ResumeScanInProgress
*	Resumes the current scan.  This function sends a resume command to the laser, but currently the laser does not support pausing of a scan, so will not resume.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system is successfully able to resume the scan in progress.
*		MIRcatSDK_RET_NOT_INITIALIZED - If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_RESUME_SCAN_FAILURE - *[System Error]* If the system fails to resume the scan in progress.
*		MIRcatSDK_RET_NO_SCAN_INPROGRESS - *[User Error]* If the user attempts to resume a scan when there is no current scan in progress.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_ResumeScanInProgress();

/* Function: ManualStepScanInProgress
*	Tells the laser to go to the next step in a Step and Measure or Multi-Spectral scan if the process trigger mode is set to Manual.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - The system successfully moves to the next step.
*		MIRcatSDK_RET_NOT_INITIALIZED - If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_MANUAL_STEP_SCAN_FAILURE - *[System Error]* If the system fails to manually move to the next step.
*		MIRcatSDK_RET_NO_SCAN_INPROGRESS - *[User Error]* If the user attempts to manually step a scan when there is no current scan in progress.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_ManualStepScanInProgress();

/* Group: Tune APIs */

// TODO: RudyB 5/23/17 - Is this function in the right group? This does not look like a tuning API function?
/* Function: GetActualWW
*	Gets the actual tuned wavelength.  This can be used during a sweep or tune to indicate when target is reached.
*
*	Parameters:
*		pfActualWW - (out) The actual wavelength the laser is currently tuned to.
*		pbUnits - (out) The wavelength units for the tuning.
*		pbLightValid - (out) Indicates if laser light is valid (tuned and emitting).
*
*	See Also:
*		<Units>
*
...
*		pbUnits - (out) The wavelength units for the tuning.
*		pbPreferredQcl - (out) The preferred QCL as specified in the last TuneToWW command indexed 1-4.  A value of 0 indicates no preferred QCL.
*
*	See Also:
*		<Units>
*
...
*		bPreferredQcl - The preferred QCL for this tune command indexed 1-4.  A value of 0 indicates no preferred QCL.
*
*	See Also:
*		<Units>
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system is successfully able to tune to the user specified wavelength.
*		MIRcatSDK_RET_NOT_INITIALIZED - If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_TECS_NOT_AT_SET_TEMPERATURE - *[System Error]* If the system is not operating at the set temperature.
*		MIRcatSDK_RET_STARTTUNE_FAILURE - *[System Error]* If the system fails to tune the laser to the user specified wavelength.
*		MIRcatSDK_RET_LASER_NOT_ARMED - *[User Error]* If the user attempts to tune the laser when the laser is not yet armed.
*		MIRcatSDK_RET_WW_OUTOFTUNINGRANGE - *[User Error]* If the user specified wavelength is out of the valid range.
*/
...
*
*	Parameters:
*		pbIsTuned - (out) Bool value that indicates if the laser is currently tuned.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*	Cancel the current single tune.  If the laser is tuned in single tune mode, this command must be sent before performing a scan.
*
*	See Also:
*		<StopScanInProgress>
*
...
*	Turns laser emission on.  Laser must have been tuned to a wavelength prior to sending this command.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system is successfully able to enable the laser emission.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_LASER_NOT_ARMED - *[User Error]* If the user attempts to enable laser emission before the laser is armed.
*		MIRcatSDK_RET_EMISSION_ALREADY_ON - *[User Error]* If the user attempts to enable the laser emission while the laser is already emitting.
*		MIRcatSDK_RET_LASER_NOT_TUNED - *[User Error]* If the user attempts to enable laser emission before tuning the laser.
*		MIRcatSDK_RET_EMISSION_ON_FAILURE - *[System Error]* If the system fails to enable the laser emission.
*/
...
*	Turns laser emission off.  If the laser emission is already off, this will return error *MIRcatSDK_RET_EMISSION_ALREADY_OFF*.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system is successfully able to disable the laser emission.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_EMISSION_ALREADY_OFF - *[User Error]* If the user attempts to diable laser emission when it is already disabled.
*		MIRcatSDK_RET_EMISSION_OFF_FAILURE - *[System Error]* If the system fails to disable the laser emission.
*/
...
*	Gets the sweep speed in the indicated units per second from the last sweep scan.
*
*	Parameters:
...
*
*	Parameters:
*		pwNumScans - (out) Number of scan iterations to be performed.  Note: A bi-directional scan will count the scan up and scan down as a single scan.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*
*	Parameters:
*		pbIsBidirectional - (out) Bool value indicating if this scan is bi-directional.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*		bIsBiDirectional - Bool value indicating if this scan is bi-directional.
*		u8PreferredQcl - The preferred QCL for this sweep scan command indexed 1-4.  A value of 0 indicates no preferred QCL.  If there is a preferred QCL, the scan will switch to this QCL as soon as possible and stay on this QCL as long as possible.
*
*	See Also:
*		<Units>
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully starts a Sweep Scan.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_LASER_NOT_ARMED - *[User Error]* If the user attempts to enable laser emission before the laser is armed.
*		MIRcatSDK_RET_WW_OUTOFTUNINGRANGE - *[User Error]* If the user specified wavelength is out of the valid range.
*		MIRcatSDK_RET_START_SWEEPSCAN_FAILURE - *[System Error]* If the system fails to start a Sweep Scan.
*		MIRcatSDK_RET_TECS_NOT_AT_SET_TEMPERATURE - *[System Error]* If the system is not operating at the set temperature.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_StartSweepScan(float fStartWW, float fStopWW, float fScanSpeed,
...
*	Gets the Step and Measure start wavelength from the last Step and Measure scan.
*
*	Parameters:
*		pfStartWW - (out) Start wavelength of the last scan.
*		pbUnits - (out) Wavelength units.
...
*	Gets the Step and Measure stop wavelength from the last Step and Measure scan.
*
*	Parameters:
*		pfStopWW - (out) Stop wavelength of the last scan.
*		pbUnits - (out) Wavelength units.
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully starts a step and measure scan.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_LASER_NOT_ARMED - *[User Error]* If the user attempts to enable laser emission before the laser is armed.
*		MIRcatSDK_RET_WW_OUTOFTUNINGRANGE - *[User Error]* If the user specified wavelength is out of the valid range.
*		MIRcatSDK_RET_TECS_NOT_AT_SET_TEMPERATURE - *[System Error]* If the system is not operating at the set temperature.
...
*
*	Parameters:
*		pbNumElements - (out) Number of elements in the Multi-Spectral scan.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the parameters for the user specified element
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_INDEX_OUTOFBOUNDS - *[User Error]* If the user specified Element Index is invalid and out of index bounds.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_GetMultiSpectralElement(uint8_t bIndex, float * pfScanWW, uint32_t * pdwDwellTime, uint32_t * pdwOffTime);
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully sets the number of elements for a Multi-Spectral scan.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_TOO_MANY_ELEMENTS - *[User Error]* The user specified number of elements is too large.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully adds an element to the end of the element list.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_INDEX_OUTOFBOUNDS - *[User Error]* If the user specified Element Index is invalid and out of index bounds.
*		MIRcatSDK_RET_WW_OUTOFTUNINGRANGE - *[User Error]* If the user specified wavelength is out of the valid range.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_AddMultiSpectralElement(float fScanWW, uint8_t bUnits, uint32_t dwDwellTime, uint32_t dwOffTime);

/* Function: StartMultiSpectralModeScan
*	Starts a Multi-Spectral scan with the specified number of iterations.  The elements for this scan must have been setup previously.
*
*	Parameters:
*		wNumScans - Number of iterations for this Multi-Spectral scan.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully starts a Multi-Spectral Scan.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_LASER_NOT_ARMED - *[User Error]* If the user attempts to enable laser emission before the laser is armed.
*		MIRcatSDK_RET_NOT_ENOUGH_ELEMENTS - *[User Error]* If the user does not define enough Multi-Spectral scan elements.
*		MIRcatSDK_RET_TECS_NOT_AT_SET_TEMPERATURE - *[System Error]* If the system is not operating at the set temperature.
...
*	Gets the number of user favorites that have been saved in the laser memory.
*
*	Parameters:
*		pbNumFavorites - (out) Number of favorites saved in the laser memory.
*
*	Returns:
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
...
*		pszFavoriteName - (out) Name of the favorite.  This character array should be 32 bytes.
*		bSize - Length in bytes of the favorite name.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the name of the favorite at the user specified index.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_INDEX_OUTOFBOUNDS - *[User Error]* If the user specified index is invalid and out of bounds.
*		MIRcatSDK_RET_BUFFER_TOO_SMALL - *[User Error]* If the user specified buffer is too small for the character array.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully recalls the favorite with the user specified name.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_FAVORITE_NAME_NOTRECOGNIZED - *[User Error]* If the user specifies an invalid favorite name.
*		MIRcatSDK_RET_FAVORITE_RECALL_FAILURE - *[System Error]* If the system fails to recall the favorite with the user specified name.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the pulse rate of the specified QCL.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the pulse width of the specified QCL.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the QCL Current of the specified QCL.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
...
*		MIRcatSDK_RET_SAVE_SETTINGS_FAILURE - *[System Error]* If the system fails to save the QCL settings.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_SetQCLParams(uint8_t bQcl, float fPulseRateInHz, float fPulseWidthInNanoSec, float fCurrentInMilliAmps);
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the QCL Pulse Limits of the specified QCL.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_GetQCLPulseLimits(uint8_t bQcl, float * pfpfPulseRateMaxInHz, float * pfPulseWidthMaxInNanoSec, float * pfDutyCycleMax);
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the Max Pulse Current of the specified QCL.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
...
*	Parameters:
*		bQcl - QCL number indexed from 1-4.
*		pbCwAllowed - (out) Bool value that indicates if CW is supported on this channel.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the status of CW being supported for the specified QCL.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
...
*	Parameters:
*		bQcl - QCL number indexed from 1-4.
*		pbFiltersInstalled - (out) Bool value that indicates if CW filters are installed on this channel.
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the status of CW filters being supported for the specified QCL.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the TEC current for the specified TEC.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the QCL temperature for the specified QCL.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the QCL operating mode for the specified channel.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the QCL set temperature for the specified channel.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the QCL temperature range for the specified channel.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*/
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully sets and saves the operating parameters for the specified QCL.
*		MIRcatSDK_RET_NOT_INITIALIZED - *[User Error]* If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_QCL_NUM_OUTOFRANGE - *[User Error]* If the user specified QCL is out of range. Must be 1-4.
*		MIRcatSDK_RET_PULSERATE_OUTOFRANGE - *[User Error]* If the user specified pulse rate is invalid and out of range.
*		MIRcatSDK_RET_PULSEWIDTH_OUTOFRANGE - *[User Error]* If the user specified pulse width is invalid and out of range.
...
*		MIRcatSDK_RET_SAVE_SETTINGS_FAILURE - *[System Error]* If the system fails to save the QCL settings.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_SetAllQclParams(uint8_t bQcl, float fPulseRate, float fPulseWidth, float fCurrentInMilliAmps, float fTemperature, uint8_t u8laserMode);
...
*		pDwellTime - (out) The amount of time (in microseconds) to remain on a wavelength during a step and measure scan.
*		pAfterOffTime - (out) Interval of time (in microseconds) for the timer that controls the delay between steps.
*
*
*	See Also:
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully gets the wavelength trigger parameters.
*		MIRcatSDK_RET_NOT_INITIALIZED - [User Error] If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_COMM_ERROR - [System Error] If the system fails to communicate with the MIRcat laser.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_GetWlTrigParams( uint8_t * pbPulseMode, uint8_t * pbProcTrigMode, float * pfWlTrigStart, float * pfWlTrigStop, float * pfWlTrigInterval, uint8_t * pbUnits, uint32_t * pDwellTime, uint32_t * pAfterOffTime );
...
*
*	Returns:
*		MIRcatSDK_RET_SUCCESS - If the system successfully sets the wavelength trigger parameters.
*		MIRcatSDK_RET_NOT_INITIALIZED - [User Error] If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_COMM_ERROR - [System Error] If the system fails to communicate with the MIRcat laser.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_SetWlTrigParams( uint8_t pbPulseMode, uint8_t pbProcTrigMode, float pfWlTrigStart, float pfWlTrigStop, float pfWlTrigInterval, uint8_t pbUnits, uint32_t pDwellTime, uint32_t pAfterOffTime );
...
*		MIRcatSDK_RET_SUCCESS - Reason
*		MIRcatSDK_RET_NOT_INITIALIZED - If the MIRcat controller is not yet initialized.
*		MIRcatSDK_RET_COMM_ERROR - Reason
*/
	MIRCAT_LIB uint32_t MIRcatSDK_GetWlTrigChanParams(uint8_t bChan, uint8_t *units, float *start_ww, float *stop_ww, float *spacing, uint16_t *numTrigs);
...
*		Always returns *MIRcatSDK_RET_SUCCESS* if the controller is initialized, else returns *MIRcatSDK_RET_NOT_INITIALIZED*.
*/
	MIRCAT_LIB uint32_t MIRcatSDK_GetSystemTemperatures( float * pfBenchTemp1, float * pfBenchTemp2, float * pfPcbTemp );


	MIRCAT_LIB uint32_t MIRcatSDK_ReadWriteAdvancedSweepParams( bool fWrite );
	MIRCAT_LIB uint32_t MIRcatSDK_SetAdvancedSweepParams( uint8_t pbUnits, float pfStartWave, float pfStopWave, float pfSpeed,
																	 uint16_t psNumScans, bool pbBidirectional );

	MIRCAT_LIB uint32_t MIRcatSDK_GetAdvancedSweepParams( uint8_t *pbUnits, float *pfStartWave, float *pfStopWave, float *pfSpeed,
																	 uint16_t *psNumScans, bool *pbBidirectional );

	MIRCAT_LIB uint32_t MIRcatSDK_SetAdvancedSweepChanParams( uint8_t bQcl, float chStartWave, float chStopWave, bool useChannel );
	MIRCAT_LIB uint32_t MIRcatSDK_GetAdvancedSweepChanParams( uint8_t bQcl, float *chStartWave, float *chStopWave, bool *useChannel );
